...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${GCX_BACKEND}/api/tv/upload/video`, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      console.error('TV video upload backend error:', res.status, text)
    ...
    console.error('TV video upload error:', error)
    return NextResponse.json(
